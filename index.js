...
const { customMessage: GiftedMess, DataBase: GiftedDB } = require('./roleen');
const gifteddb = new GiftedDB();
let Gifted;
const app = express();

app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, './roleen/roleen.html'));
});
...
async function startGifted() {
    if (!Gifted) {
        ...
        console.log(chalk.bgHex('#90EE90').hex('#333').bold(' Rollen V1 Connected '));
